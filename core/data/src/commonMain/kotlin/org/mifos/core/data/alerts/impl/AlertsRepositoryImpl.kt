 ...
 *
 * Field-level mapping:
 * - [symbol] → [PriceAlert.coinId] (both identify the watched instrument)
 * - [conditionAbove] → [PriceAlert.direction] (true → ABOVE, false → BELOW)
 * - [targetPrice] → [PriceAlert.targetValue]
 * - [createdAt] → [PriceAlert.createdAtMs]
 * - [PriceAlert.enabled] defaults to `true` (AlertEntity has no enabled field)
 */
private fun AlertEntity.toPriceAlert(): PriceAlert =
    PriceAlert(
        id = id,
        coinId = symbol,
 ...
 *
 * Field-level mapping:
 * - [coinId] → [AlertEntity.symbol]
 * - [direction] → [AlertEntity.conditionAbove] (ABOVE → true, BELOW/PCT_CHANGE → false)
 * - [targetValue] → [AlertEntity.targetPrice]
 * - [createdAtMs] → [AlertEntity.createdAt]
 */
private fun PriceAlert.toAlertEntity(): AlertEntity =
    AlertEntity(
        id = id,
        symbol = coinId,
