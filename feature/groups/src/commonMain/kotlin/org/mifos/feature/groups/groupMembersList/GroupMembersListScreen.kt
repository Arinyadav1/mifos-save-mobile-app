...
fun GroupMembersListScreen(
    ...
    EventsEffect(viewModel.eventFlow) { event ->
        when (event) {
            ...
            is GroupMembersListEvent.NavigateToAddMembers -> onAddMembersClick(
                event.groupId,
                event.officeId,
            )
        }
...
internal fun GroupMembersListScreenContent(
    ...
    KptScaffold(
        ...
        bottomBar = {
            if (state.isSelectionMode &&
                state.submitState is SubmitState.Idle ||
                state.submitState is SubmitState.Submitting
            ) {
                KptDoubleButton(
                    onLeftButtonClick = { onAction(GroupMembersListAction.ClearSelection) },
                    onRightButtonClick = { onAction(GroupMembersListAction.ShowRemoveConfirmation) },
                    leftButtonText = stringResource(Res.string.feature_groups_cancel),
                    rightButtonText = stringResource(Res.string.feature_groups_remove_members),
                )
            }
        },
    ) {
        Box(
            ...
            Column(modifier = Modifier.fillMaxSize()) {
                ...
                MutationScreenContent(
                    ...
                    Column(modifier = Modifier.fillMaxSize()) {
                        LazyColumn(
                            modifier = Modifier.fillMaxSize().padding(bottom = KptTheme.spacing.md),
